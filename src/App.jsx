...
const hoy = new Date();
const lim = new Date(); lim.setDate(hoy.getDate() + 30);
...
return recs
.filter((v) => { if (!v.refuerzo) return false; const d = new Date(v.refuerzo); return d >= hoy && d <= lim; })
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 20 }}>
          {filteredPatients.map((p) => (
            <div key={p.id} style={{ background: "#fff", padding: 25, borderRadius: 25, border: p.alergias ? "2px solid #ff4d4d" : "1px solid #eee" }}>
              ...
              {/* SEXO Y EDAD visibles en la tarjeta */}
              {(p.sexo || p.edad) && (
      ...
      <div style={{ background: "#fff", borderRadius: 30, width: "100%", maxWidth: 750, maxHeight: "95vh", overflowY: "auto", padding: 25 }}>
        ...
        <div style={{ border: "1px solid #d8e8d0", borderRadius: 15, padding: 15, marginBottom: 15 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginTop: 5 }}>
            ...
            <input placeholder="TLLC" value={cForm.tllc} style={inp} onChange={(e) => setCForm((f) => ({ ...f, tllc: e.target.value }))} />
            <select style={inp} value={cForm.cc} onChange={(e) => setCForm((f) => ({ ...f, cc: e.target.value }))}>
              {["1", "2", "3", "4", "5"].map((n) => <option key={n} value={n}>{"CC " + n + "/5"}</option>)}
            </select>
          </div>
          <input placeholder="Cavidad Oral / Otros" style={inp} value={cForm.oral} onChange={(e) => setCForm((f) => ({ ...f, oral: e.target.value }))} />
        </div>
        ...
        <label style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 15, cursor: "pointer", fontSize: 13 }}>
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "#fff", padding: 30, borderRadius: 30, width: "90%", maxWidth: 520, maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          ...
          <div>
            <span style={labelS}>ESPECIE</span>
        ...
        {/* Fila 2: Raza */}
        <span style={labelS}>RAZA</span>
        <input placeholder="Ej: Labrador, Mestizo" value={pForm.breed} style={inp} onChange={(e) => setPForm((f) => ({ ...f, breed: e.target.value }))} />

        {/* Fila 3: Sexo + Estado reproductivo */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <div>
            <span style={labelS}>SEXO</span>
            <select value={pForm.sexo} style={inp} onChange={(e) => setPForm((f) => ({ ...f, sexo: e.target.value }))}>
              <option value="Macho">Macho</option>
              <option value="Hembra">Hembra</option>
            </select>
          </div>
          <div>
            <span style={labelS}>ESTADO REPRODUCTIVO</span>
        ...
        {/* Fila 4: Edad + Unidad + Peso */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
          <div>
            <span style={labelS}>EDAD</span>
            <input
              placeholder="Ej: 3"
              type="number"
              min="0"
              value={pForm.edad}
              style={inp}
              onChange={(e) => setPForm((f) => ({ ...f, edad: e.target.value }))}
            />
          </div>
          <div>
            <span style={labelS}>UNIDAD</span>
            <select value={pForm.edadUnidad} style={inp} onChange={(e) => setPForm((f) => ({ ...f, edadUnidad: e.target.value }))}>
          ...
          <div>
            <span style={labelS}>PESO (kg)</span>
            <input placeholder="Ej: 8.5" value={pForm.weight} type="number" style={inp} onChange={(e) => setPForm((f) => ({ ...f, weight: e.target.value }))} />
          </div>
        </div>

        {/* Alergias */}
        <span style={labelS}>ALERGIAS CONOCIDAS</span>
        <input placeholder="Ej: Penicilina" style={{ ...inp, border: "1.5px solid #ff4d4d" }} value={pForm.alergias} onChange={(e) => setPForm((f) => ({ ...f, alergias: e.target.value }))} />

        {/* Tutor */}
        <div style={{ borderTop: "1px solid #eee", marginTop: 8, paddingTop: 12 }}>
          <span style={{ ...labelS, fontSize: "12px", color: "#1a331a" }}>DATOS DEL TUTOR</span>
        </div>
        <span style={labelS}>NOMBRE TUTOR</span>
        <input placeholder="Nombre completo" value={pForm.ownerName} style={inp} onChange={(e) => setPForm((f) => ({ ...f, ownerName: e.target.value }))} />
        <span style={labelS}>WHATSAPP</span>
        <input placeholder="9 XXXX XXXX" value={pForm.ownerPhone} style={inp} onChange={(e) => setPForm((f) => ({ ...f, ownerPhone: e.target.value }))} />
        <span style={labelS}>DIRECCION</span>
        <input placeholder="Calle, numero, comuna" value={pForm.ownerAddress} style={inp} onChange={(e) => setPForm((f) => ({ ...f, ownerAddress: e.target.value }))} />
  ...
  {modal === "vacuna" && activePat && (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "#fff", padding: 30, borderRadius: 25, width: "90%", maxWidth: 400 }}>
        <h3 style={{ margin: "0 0 15px" }}>{"Vac/Parasitario - " + activePat.name}</h3>
        ...
        <input placeholder="Ej: Octuple, Bravecto, Milbemax" value={vForm.nombre} style={inp} onChange={(e) => setVForm((f) => ({ ...f, nombre: e.target.value }))} />
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <div>
            <span style={labelS}>APLICADO</span>
            <input type="date" value={vForm.fecha} style={inp} onChange={(e) => setVForm((f) => ({ ...f, fecha: e.target.value }))} />
          </div>
          <div>
            <span style={labelS}>PROX. REFUERZO</span>
            <input type="date" value={vForm.refuerzo} style={inp} onChange={(e) => setVForm((f) => ({ ...f, refuerzo: e.target.value }))} />
          </div>
        </div>
        <button style={{ ...btnG, width: "100%", marginTop: 10 }} onClick={() => {
          if (!vForm.nombre || !vForm.refuerzo) { alert("Ingresa nombre y fecha de refuerzo"); return; }
          const listKey = vForm.tipo === "vacuna" ? "vacunas" : "parasitos";
          setPatients((prev) => prev.map((p) => p.id === activePatId ? { ...p, [listKey]: [{ ...vForm, id: Date.now() }, ...(p[listKey] || [])] } : p));
          setModal(null);
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "#fff", padding: 35, borderRadius: 30, width: "90%", maxWidth: 650, maxHeight: "85vh", overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          ...
          <span style={{ fontSize: 12, background: "#f0f5ef", padding: "5px 10px", borderRadius: 10 }}>{(activePat.history && activePat.history.length) || 0} atenciones</span>
        </div>

        {/* Resumen del paciente */}
        <div style={{ background: "#f9fdf7", border: "1px solid #e0ede0", borderRadius: 12, padding: "10px 14px", marginBottom: 16, fontSize: 13 }}>
          <strong style={{ color: "#1a331a" }}>{activePat.name}</strong>
          {" — "}{activePat.species}{activePat.breed ? " / " + activePat.breed : ""}
          {activePat.sexo ? "  |  " + activePat.sexo : ""}
          {activePat.edad ? "  |  " + activePat.edad + " " + (activePat.edadUnidad || "años") : ""}
          {activePat.weight ? "  |  " + activePat.weight + " kg" : ""}
          {activePat.reproductivo ? "  |  " + activePat.reproductivo : ""}
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 15, marginBottom: 20, background: "#f9f9f9", padding: 15, borderRadius: 15 }}>
          <div>
            <span style={labelS}>ULTIMAS VACUNAS</span>
            {activePat.vacunas && activePat.vacunas.length > 0 ? activePat.vacunas.slice(0, 2).map((v, i) => <div key={i} style={{ fontSize: 11 }}>{"- " + v.nombre + " (" + v.fecha + ")"}</div>) : <div style={{ fontSize: 10, color: "#aaa" }}>Sin registros</div>}
          </div>
          <div>
            <span style={labelS}>ULTIMOS PARASITOS</span>
            {activePat.parasitos && activePat.parasitos.length > 0 ? activePat.parasitos.slice(0, 2).map((v, i) => <div key={i} style={{ fontSize: 11 }}>{"- " + v.nombre + " (" + v.fecha + ")"}</div>) : <div style={{ fontSize: 10, color: "#aaa" }}>Sin registros</div>}
          </div>
        </div>

        {(!activePat.history || activePat.history.length === 0) ? (
          <p style={{ color: "#aaa" }}>Sin registros clinicos.</p>
        ) : (
