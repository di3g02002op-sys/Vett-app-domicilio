...
{ label:“Infección urinaria”,   texto:“1. Enrofloxacino 5mg/kg c/24h por 10 días.\n2. Aumentar consumo de agua (añadir agua a croquetas).\n3. Urocultivo si no mejora en 5 días.\n4. Evitar alimentos con mucho fósforo.\nControl en 10 días con sedimento urinario.” },
];

// ─── PDF ─────────────────────────────────────────────────────────────
const exportPDF = (p, type = “historia”, consulta = null) => {
const isReceta = type === “receta”;
const html = `<html><head><style> body{font-family:'Segoe UI',sans-serif;padding:40px;color:#333;line-height:1.6} .header{display:flex;justify-content:space-between;align-items:center;border-bottom:4px solid #1a331a;padding-bottom:15px;margin-bottom:25px} .info-grid{background:#f9f9f9;padding:20px;border-radius:12px;margin-bottom:25px;display:grid;grid-template-columns:1fr 1fr;gap:15px;border:1px solid #eee} .alerta{color:#d32f2f;font-weight:bold;border:2px solid #d32f2f;padding:8px;margin-top:10px;border-radius:8px;text-align:center;background:#fff1f1} .footer{margin-top:60px;text-align:center;font-size:11px;border-top:1px solid #ddd;padding-top:15px;color:#777} h3{color:#1a331a;border-bottom:2px solid #3a7a3a;padding-bottom:5px;margin-top:30px} .med-box{font-size:15px;white-space:pre-wrap;padding:25px;border:2px dashed #3a7a3a;border-radius:15px;background:#fff} .legal{font-size:10px;color:#666;margin-top:30px;border:1px solid #eee;padding:10px} .exam-grid{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;font-size:12px} </style></head><body> <div class="header"> <img src="${LOGO_URL}" style="height:70px" onerror="this.style.display='none'"> <div style="text-align:right"> <h1 style="margin:0;color:#1a331a">${isReceta?"RECETA MÉDICA":"FICHA CLÍNICA"}</h1> <h2 style="margin:0;color:#3a7a3a">${CLINICA}</h2> <p style="margin:5px 0">${DOCTOR}</p> </div> </div> <div class="info-grid"> <div><strong>Tutor:</strong> ${p.ownerName}<br><strong>WhatsApp:</strong> ${p.ownerPhone}<br><strong>Dirección:</strong> ${p.ownerAddress}</div> <div><strong>Paciente:</strong> ${p.name}<br><strong>Especie/Raza:</strong> ${p.species}/${p.breed}<br> <strong>Peso:</strong> ${consulta?.weight||p.weight||"--"} kg | <strong>Estado:</strong> ${p.reproductivo||"Entero/a"} ${p.alergias?`<div class="alerta">⚠️ ALÉRGICO A: ${p.alergias.toUpperCase()}</div>`:""} </div> </div> ${isReceta ?`<h3>INDICACIONES:</h3><div class="med-box">${consulta.tratamiento}</div>` :`<h3>ANAMNESIS:</h3><p>${consulta.anamnesis||“Sin registros.”}</p>
<h3>EXAMEN FÍSICO (${consulta.date})</h3>
<div class="exam-grid">
<div><b>Peso:</b> ${consulta.weight||”–”} kg</div><div><b>T°:</b> ${consulta.temp||”–”} °C</div>
<div><b>FC:</b> ${consulta.fc||”–”} lpm</div><div><b>FR:</b> ${consulta.fr||”–”} rpm</div>
<div><b>TLLC:</b> ${consulta.tllc||”–”} seg</div><div><b>Mucosas:</b> ${consulta.mucosas||”–”}</div>
<div><b>CC:</b> ${consulta.cc||”–”}/5</div><div><b>Linfonodos:</b> ${consulta.linfonodos||”–”}</div>
<div><b>Hidratación:</b> ${consulta.hidratacion||”–”} %</div>
</div>
<h3>DIAGNÓSTICO</h3><p>${consulta.diagnostico}</p>
<h3>PLAN TERAPÉUTICO</h3><p style="white-space:pre-wrap">${consulta.tratamiento}</p>` } ${!isReceta&&consulta?.consentimiento ?`<div class="legal"><b>CONSENTIMIENTO INFORMADO:</b> El tutor declara haber sido informado de los riesgos y autoriza al ${DOCTOR} a proceder.<br><br><br>__________________________<br>Firma del Tutor</div>` :""} <div class="footer"><p>Concepción, Chile.${consulta?.proximoControl?` | Próximo Control: ${consulta.proximoControl}`:””}</p></div>
...
Cualquier consulta estoy a su disposición. ¡Que ${p.name} se recupere pronto! 🐶❤️`;
...
const [statsRange,   setStatsRange]  = useState(“3”);   // meses a mostrar
...
const hoy=new Date(), lim=new Date(); lim.setDate(hoy.getDate()+30);
return patients.flatMap(p=>{
const recs=[…(p.vacunas||[]),…(p.parasitos||[])];
if(p.history?.[0]?.proximoControl) recs.push({nombre:“Control Clínico”,refuerzo:p.history[0].proximoControl});
return recs.filter(v=>{const d=new Date(v.refuerzo);return d>=hoy&&d<=lim;})
...
const consultasMes = Object.entries(byMonth).sort(([a],[b])=>a.localeCompare(b))
...
const byDx = {};
recent.forEach(h=>{
  if(h.diagnostico){ const k=h.diagnostico.trim(); byDx[k]=(byDx[k]||0)+1; }
});
...
// ── Curva de peso del paciente activo ──
...
const a    = document.createElement(“a”); a.href=url; a.download=`vet_backup_${new Date().toISOString().slice(0,10)}.json`; a.click();
};
...
if(window.confirm(`Restaurar ${data.patients.length} pacientes y ${data.finances.length} movimientos?\nEsto reemplazará los datos actuales.`)){
setPatients(data.patients); setFinances(data.finances);
}
...
const openNewPat = () => { setPForm({name:””,species:“Perro”,breed:””,weight:””,reproductivo:“Entero/a”,alergias:””,ownerName:””,ownerPhone:””,ownerAddress:””}); setActivePatId(null); setModal(“paciente”); };
...
```
  ...
  <header style={{background:"#1a331a",color:"#fff",padding:"15px 25px",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,zIndex:100,boxShadow:"0 2px 12px rgba(0,0,0,0.3)"}}>
    ...
    <nav style={{display:"flex",gap:6}}>
      {NAV.map(n=>(
        <button key={n.k} onClick={()=>setTab(n.k)} style={{background:tab===n.k?"#3a7a3a":"transparent",border:tab===n.k?"none":"1px solid rgba(255,255,255,.2)",color:"#fff",cursor:"pointer",fontWeight:700,fontSize:13,borderRadius:10,padding:"8px 14px",display:"flex",alignItems:"center",gap:5}}>
          {n.i} {n.l}
        </button>
      ))}
    </nav>
  ...
  <main style={{padding:20,maxWidth:920,margin:"0 auto"}}>
    ...
    {tab==="inicio"&&<>
      ...
      <div style={{background:"#fff",padding:25,borderRadius:20,marginBottom:16,boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <h3 style={{margin:0}}>📢 Recordatorios próximos (30 días)</h3>
          {alertas.length>0&&<span style={{background:"#e67e22",color:"#fff",borderRadius:20,padding:"2px 12px",fontSize:12,fontWeight:700}}>{alertas.length}</span>}
        </div>
        {alertas.length===0
          ...
          :alertas.map((a,i)=>(
            <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:"1px solid #f0f0f0"}}>
              <div>
                ...
                <br/><small style={{color:"#888"}}>📅 {a.refuerzo} · {a.tutor}</small>
              </div>
              <button onClick={()=>window.open(`https://wa.me/56${a.phone.replace(/\D/g,"")}?text=Hola ${a.tutor}, le recuerdo que ${a.pName} tiene su ${a.nombre} para el ${a.refuerzo}.`)} style={{...btnG,background:"#25D366",padding:"8px 14px",fontSize:11}}>
                WhatsApp
              </button>
            </div>
          ))
      ...
      <div style={{background:"#fff",padding:20,borderRadius:20,boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}>
        ...
        <p style={{margin:"10px 0 0",fontSize:12,color:"#aaa"}}>Exporta para hacer copias de seguridad o trasladar datos entre dispositivos.</p>
      </div>
    </>}
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(290px,1fr))",gap:18}}>
        {filteredPats.map(p=>(
          <div key={p.id} style={{background:"#fff",padding:22,borderRadius:22,border:p.alergias?"2px solid #ff4d4d":"1.5px solid #e8f0e8",boxShadow:"0 2px 8px rgba(0,0,0,0.05)"}}>
            ...
            {p.alergias&&<div style={{background:"#fff1f1",border:"1px solid #ff4d4d",borderRadius:8,padding:"4px 10px",fontSize:12,color:"#d32f2f",marginBottom:8}}>⚠️ {p.alergias}</div>}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7}}>
              <button onClick={()=>openConsulta(p)} style={btnG}>🩺 Consulta</button>
              <button onClick={()=>openVacuna(p)}   style={{...btnG,background:"#4a90e2"}}>💉 Vac/Parás</button>
              <button onClick={()=>openHist(p)}     style={{...btnG,gridColumn:"span 2",background:"#f0f5ef",color:"#3a7a3a"}}>
                📜 Historial ({p.history?.length||0})
              </button>
            </div>
          </div>
        ))}
    ...
    {tab==="stats"&&<>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        <h2 style={{margin:0,fontFamily:"serif",color:"#1a331a"}}>📊 Estadísticas Clínicas</h2>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:14,marginBottom:22}}>
        {[
          ...
          {l:"Pacientes activos",v:patients.length,c:"#3a7a3a"},
          {l:"Alertas próximas",v:alertas.length,c:"#e67e22"},
        ].map(s=>(
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
        ...
        <div style={{background:"#fff",padding:22,borderRadius:18,boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}>
          ...
          {clinStats.consultasMes.length===0
            ?<p style={{color:"#aaa",fontSize:13}}>Sin datos en este período.</p>
              ...
              return(
                <div key={sp} style={{marginBottom:10}}>
                  ...
                  <div style={{background:"#f0f5ef",borderRadius:20,height:8}}>
                    <div style={{background:"#3a7a3a",borderRadius:20,height:8,width:`${Math.round(n/total*100)}%`,transition:"width 0.4s"}}/>
                  </div>
                </div>
              );
            })
      ...
      {/* Diagnósticos más frecuentes */}
      <div style={{background:"#fff",padding:22,borderRadius:18,boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}>
        <h4 style={{margin:"0 0 14px",color:"#1a331a"}}>🔬 Diagnósticos más frecuentes</h4>
        {clinStats.topDx.length===0
          ?<p style={{color:"#aaa",fontSize:13}}>Sin diagnósticos registrados en este período.</p>
      ...
      <div style={{background:"#fff",padding:28,borderRadius:22,boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20,flexWrap:"wrap",gap:10}}>
          ...
          <div style={{display:"flex",gap:8}}>
            <button onClick={exportCSV} style={{background:"#1D6F42",color:"#fff",border:"none",padding:"8px 14px",borderRadius:10,cursor:"pointer",fontSize:12}}>📥 CSV</button>
            <button onClick={exportBackup} style={{background:"#4a90e2",color:"#fff",border:"none",padding:"8px 14px",borderRadius:10,cursor:"pointer",fontSize:12}}>💾 Respaldo</button>
          </div>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gap:10,marginBottom:10}}>
          <input placeholder="Descripción" style={inp} value={fForm.desc} onChange={e=>setFForm(f=>({...f,desc:e.target.value}))}/>
        ...
        {finances.map((f,i)=>(
          <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:"1px solid #f0f0f0"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              ...
              <button onClick={()=>setFinances(prev=>prev.filter((_,j)=>j!==i))} style={{background:"none",border:"none",color:"#e74c3c",cursor:"pointer",fontWeight:700,fontSize:16}}>×</button>
            </div>
          </div>
        ))}
      </div>
  ...
  {modal==="consulta"&&activePat&&(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.82)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:10}}>
      <div style={{background:"#fff",borderRadius:28,width:"100%",maxWidth:760,maxHeight:"95vh",overflowY:"auto",padding:25}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <h3 style={{margin:0}}>{activeHistId?"Editar Atención":"Nueva Atención"}: {activePat.name}</h3>
        ...
        <div style={{border:"1px solid #d8e8d0",borderRadius:14,padding:14,marginBottom:14}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
            {[["Peso(kg)","weight"],["T°(°C)","temp"],["FC(lpm)","fc"],["FR(rpm)","fr"]].map(([l,k])=>(
              <div key={k}><small style={{fontSize:"9px",color:"#666"}}>{l}</small><input value={cForm[k]} style={inp} onChange={e=>setCForm(f=>({...f,[k]:e.target.value}))}/></div>
            ))}
          </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            ...
            <input placeholder="Hidratación %" value={cForm.hidratacion} style={inp} onChange={e=>setCForm(f=>({...f,hidratacion:e.target.value}))}/>
          </div>
          <input placeholder="Cavidad Oral / Otros" value={cForm.oral} style={inp} onChange={e=>setCForm(f=>({...f,oral:e.target.value}))}/>
        </div>

        <input placeholder="Diagnóstico..." style={inp} value={cForm.diagnostico} onChange={e=>setCForm(f=>({...f,diagnostico:e.target.value}))}/>
        ...
        <div style={{marginBottom:6}}>
          ...
          <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:8}}>
            {PLANTILLAS.map((pl,i)=>(
              <button key={i} onClick={()=>setCForm(f=>({...f,tratamiento:pl.texto}))}
                style={{background:"#f0f5ef",border:"1px solid #d0e8d0",borderRadius:8,padding:"4px 10px",fontSize:11,cursor:"pointer",color:"#3a7a3a",fontWeight:600}}>
                {pl.label}
              </button>
            ))}
          </div>
        ...
        <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:12}}>
          <span style={{...labelS,margin:0}}>PRÓXIMO CONTROL:</span>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:8}}>
          <button style={{...btnG,gridColumn:"span 1"}} onClick={saveConsulta}>💾 Guardar</button>
      ...
      <div style={{background:"#fff",padding:28,borderRadius:28,width:"90%",maxWidth:500,maxHeight:"90vh",overflowY:"auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          ...
          <select value={pForm.reproductivo} style={inp} onChange={e=>setPForm(f=>({...f,reproductivo:e.target.value}))}>
            <option>Entero/a</option><option>Castrado/a</option>
          </select>
        </div>
        ...
        <input placeholder="Dirección" value={pForm.ownerAddress} style={inp} onChange={e=>setPForm(f=>({...f,ownerAddress:e.target.value}))}/>
        <button style={{...btnG,width:"100%",marginBottom:8}} onClick={()=>{
          ...
          else setPatients(prev=>[{...pForm,id:Date.now(),history:[],vacunas:[],parasitos:[]},...prev]);
          setModal(null);
  ...
  {modal==="vacuna"&&activePat&&(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.82)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{background:"#fff",padding:28,borderRadius:24,width:"90%",maxWidth:420}}>
        <h3 style={{margin:"0 0 14px"}}>💉 Vac/Parás — {activePat.name}</h3>
        ...
        <input placeholder="Nombre (Ej: Óctuple, Bravecto)" style={inp} value={vForm.nombre} onChange={e=>setVForm(f=>({...f,nombre:e.target.value}))}/>
        <label style={labelS}>Fecha de aplicación</label>
        <input type="date" style={inp} value={vForm.fecha} onChange={e=>setVForm(f=>({...f,fecha:e.target.value}))}/>
        <label style={labelS}>Fecha de refuerzo / próxima dosis</label>
            ...
            <div key={i} style={{borderBottom:"1px solid #eee",padding:"14px 0"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
                <div>
                  ...
                  <span style={{marginLeft:8,fontSize:13,color:"#555"}}>Dx: {h.diagnostico||"Sin diagnóstico"}</span>
                </div>
                <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                  <button onClick={()=>openEditC(h)} style={{...btnG,padding:"5px 10px",background:"#f39c12",fontSize:11}}>✏️</button>
                  <button onClick={()=>sendWAReport(activePat,h)} style={{...btnG,padding:"5px 10px",background:"#25D366",fontSize:11}}>📲</button>
                  <button onClick={()=>exportPDF(activePat,"historia",h)} style={{...btnG,padding:"5px 9px",fontSize:11}}>📄</button>
                  <button onClick={()=>exportPDF(activePat,"receta",h)} style={{...btnG,padding:"5px 9px",fontSize:11,background:"#4a90e2"}}>💊</button>
                </div>
              </div>
